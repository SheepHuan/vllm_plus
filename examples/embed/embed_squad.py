...
def process_squad_dataset(save_path):
    dataset_path = "rajpurkar/squad"
    tokenizer = transformers.AutoTokenizer.from_pretrained("Qwen/Qwen2.5-7B-Instruct")
    train_dataset = list(datasets.load_dataset(dataset_path,split="train"))
    test_dataset = list(datasets.load_dataset(dataset_path,split="validation"))
...
def embed_squad_dataset(dataset_path):
    tokenizer = transformers.AutoTokenizer.from_pretrained("Qwen/Qwen2.5-7B-Instruct")
    dataset = json.load(open(dataset_path, "r"))
    dataset_embedder = DatasetEmbeder(
        model_name="all-MiniLM-L6-v2",
        collection_name="squad",
        database_path="examples/dataset/data/database/squad.db"
    )
     # 初始化批处理列表
    batch_items = []
    batch_texts = []
...
def find_squad_similar_docs(dataset_path,save_path):
    ...
    client = MilvusClient("/root/code/vllm_plus/examples/dataset/data/database/squad.db")
    collection_name = "squad"
    save_data = {
        "all_data": {},
        "similar_pairs": {}
    ...
    for item in tqdm(dataset):
        ...
        if len(results[0]) > 0:
            ...
            for result in results[0]:
                ...
                source_tokens = tokenizer.encode(result["entity"]["context"]+"\n"+result["entity"]["question"])
                target_tokens = tokenizer.encode(item["context"]+"\n"+item["question"])
